...
(function () {
  ...
  const CFG = {
    enemyCount: 5, radarRange: 650, lockRange: 480, lockCone: 0.92,
    lockSeconds: 2.4, lockDecay: 1.1, missileSpeed: 185,
    missileLife: 7, missileTurn: 2.25, enemyFireDelay: 7,
    respawnDelay: 12, maxParticles: 180
  };
  const enemies = [], missiles = [], particles = [];
  const state = { active:false, lock:0, target:null, missiles:6, hull:100,
  ...
  let missileLatch=false, rearLatch=false, initialized=false, gunClock=0, lockBeepClock=0, lockTonePlayed=false;
  ...
  function acquireTarget(dt){
    ...
    if(best&&best.mesh.position.distanceTo(player.position)<CFG.lockRange&&targetAlignment>CFG.lockCone){
      if(state.target!==best)state.lock=Math.min(state.lock,.25); state.target=best; state.lock=Math.min(1,state.lock+dt/CFG.lockSeconds);
      lockBeepClock-=dt;
      if(state.lock<1&&lockBeepClock<=0){tone(520+state.lock*520,.07,'square',.16);lockBeepClock=.48-state.lock*.34;lockTonePlayed=false;}
      if(state.lock>=1&&!lockTonePlayed){tone(1180,.62,'square',.14);lockTonePlayed=true;}
    }else{ state.lock=Math.max(0,state.lock-dt/CFG.lockDecay);lockBeepClock=0;lockTonePlayed=false;if(state.lock===0)state.target=best; }
  }
  ...
  function launchMissile(owner,target,forcedDirection){
    if(owner==='player'&&(state.missiles<=0||(!forcedDirection&&state.lock<1)||!target))return;
    ...
    if(owner==='player'){state.missiles--;tone(180,.35,'sawtooth',.18);} else {spatialTone(760,source.position,.24);}
  }
  ...
  function launchRearMissile(){
    if(state.missiles<=0)return;const f=forwardOf(player);let target=null,best=Infinity;
    ...
    if(target){launchMissile('player',target,f.multiplyScalar(-1));tone(420,.16,'square',.12);}else tone(120,.1,'square',.05);
  }
  function updateMissiles(dt){
    for(let i=missiles.length-1;i>=0;i--){const m=missiles[i];m.life-=dt;
      ...
      m.mesh.position.addScaledVector(m.vel,dt);m.mesh.lookAt(m.mesh.position.clone().add(m.vel)); trail(m.mesh.position,m.owner==='player'?0xb9eaff:0xff7d32);
  ...
  function damageEnemy(e,amount,hit){
    ...
    e[zone]=Math.max(0,e[zone]-amount);e.health=Math.max(0,e.health-amount);if(e.health<=0){e.dead=true;e.respawn=CFG.respawnDelay;scene.remove(e.mesh);state.score++;state.target=null;state.lock=0;}
  }
  ...
  function updateParticles(dt){for(let i=particles.length-1;i>=0;i--){const p=particles[i];p.life-=dt;p.mesh.position.addScaledVector(p.vel,dt);p.vel.y-=5*dt;p.mesh.material.opacity=Math.max(0,p.life);if(p.life<=0){scene.remove(p.mesh);p.mesh.geometry.dispose();p.mesh.material.dispose();particles.splice(i,1);}}}
  ...
  function updateHud(){if(!hud)return;hud.style.display=available()?'block':'none';if(!available())return;
    ...
    s.innerHTML=`COQUE ${Math.round(state.hull)}% &nbsp; MOTEUR ${Math.round(state.engine)}%<br>AILE G ${Math.round(state.leftWing)}% &nbsp; AILE D ${Math.round(state.rightWing)}%<br>MISSILES ${state.missiles} &nbsp; VICTOIRES ${state.score}<br>[M/B] AVANT &nbsp; [R/Y] MISSILE ARRIÈRE`;
    ...
    if(state.target&&typeof camera!=='undefined'){
      ...
      if(visible){seeker.style.left=((screen.x*.5+.5)*innerWidth)+'px';seeker.style.top=((-screen.y*.5+.5)*innerHeight)+'px';const scale=1.45-state.lock*.62;seeker.style.transform=`scale(${scale}) rotate(${state.lock*130}deg)`;seeker.style.borderColor=locked?'#ff3d32':'#ffd84a';}
